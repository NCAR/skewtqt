...
SkewTAdapterQt::Text(const std::string &s, const double x, const double y, unsigned int colorCode)
{
  ...
  if (s[0] == '|') {
    ...
    case 'r':
      ...
      break;
    case 'c':
      alignFlag = Qt::AlignHCenter | AlignVCenter;
      break;
    case 'l':
...
SkewTAdapterQt::resizeEvent( QResizeEvent *e )
{
  ...
  _resizeTimer.start(_resizeHoldOffMs, true);
}
...
{
  // if painting is allwoed, redraw the whole thing.
  if (!_dontPaint)
    if (_ready)
      drawElements();
}
...
SkewTAdapterQt::mouseReleaseEvent( QMouseEvent* e )
{
  ...
  if( _zoomStart.x() != _zoomStop.x() && _zoomStart.y() != _zoomStop.y() ) {
    ...
    drawBoundingRect( &p, _zoomStart, _zoomStop );
...
//////////////////////////////////////////////////////////////////////
void 
SkewTAdapterQt::drawBoundingRect( QPainter* p, const QPoint& p1, const QPoint& p2 )
{
  p->drawRect( 
...
SkewTAdapterQt::print(QPrinter* printer) {
    ...
    QPaintDeviceMetrics spm( this );
    QPaintDeviceMetrics pm( printer );
    double ScreenToPrinterX = (double)pm.logicalDpiX() /
                                (double)spm.logicalDpiX();
    double ScreenToPrinterY = (double)pm.logicalDpiY() /
                                (double)spm.logicalDpiY();
    ...
    // Printer Page Size
    double wPage = (double)pm.width();
    double hPage = (double)pm.height();
...
SkewTQtPolyline::draw(QPainter& painter, int width, int height)
{
  ...
  int nPoints = _x.size();
  QPointArray points = QPointArray(nPoints);
